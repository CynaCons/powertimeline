/**
 * Firestore SRS compliance checks - Development environment
 * Read-only validations against powertimeline-dev project.
 */
...
test.describe('Dev Firestore - SRS DB compliance', () => {
  test('users: uniqueness, roles, reserved, fields', async () => {
    ...
    // Admins should be limited to allowlist
  ...
  test('events: fields, timeline linkage', async () => {
    ...
    snap.forEach((doc) => {
      ...
      // parent path: users/{uid}/timelines/{timelineId}/events/{eventId}
